...
@test lg.fmt["alpha"] == EntrySpec("%3.2e",1, 1, 6)
@test lg.idx[1] == 1
...
@test lg.fmt["iter"] == EntrySpec("%10d",2,1,10)
...
@test lg.fmt["ϕ"] == EntrySpec(def[Integer],4,1, SolverLogging.getwidth(def[Integer]))
lg.data[3] = "phi"
...
@test length(SolverLogging.formrow(lg)) == 30
...
@test occursin("ϕ", SolverLogging.formheader(lg))
@test length(SolverLogging.formrow(lg)) == 42 