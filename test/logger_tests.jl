...
@test SolverLogging.cache_size(ld) == 0
SolverLogging.add_col!(ld,:c_max, 15, vartype=Float64)
@test ld.cols == [:iter,:cost,:c_max]
@test ld.widths[3] == 15
SolverLogging.add_col!(ld,:outer, 15, 2, vartype=Float64, do_print=false)
@test ld.cols == [:iter,:outer,:cost,:c_max]
@test ld.print[2] == false
SolverLogging.add_col!(ld,:cost)
@test length(ld.cols) == 4

strhead = SolverLogging.create_header(ld)
...
# Clear data
SolverLogging.clear!(ld)
@test ld.data[:iter] == ""


# Logger Tests
logger = SolverLogger(InnerLoop, default_width=15)
SolverLogging.add_level!(logger, InnerLoop, cols, width; print_color=:green, indent=4)
@test logger.leveldata[InnerLoop].indent == 4
SolverLogging.add_level!(logger, InnerLoop, cols, width; print_color=:green, indent=5)
...
with_logger(logger) do
    ...
    @test SolverLogging.cache_size(logger[InnerLoop]) == 1
    ...
    strrow = SolverLogging.create_row(logger[InnerLoop])
    ...
    SolverLogging.clear_cache!(logger[InnerLoop])
end
